...
from openpyxl.chart import BarChart, LineChart, BubbleChart, Series, Reference

# ... (Previous imports)

def create_bubble_chart(ws, title, data_range, start_row, num_cols):
    chart = BubbleChart()
    chart.title = title
    chart.style = 10
    chart.y_axis.title = "Volumen"
    chart.x_axis.title = "Precio Promedio"
    
    # Assumptions based on exportUtils data structure:
    # Col 1: _Label (Model)
    # Col 2: Marca (Brand) - Used for Series Grouping
    # Col 3: Precio Promedio (X)
    # Col 4: Volumen (Y)
    # Col 5: Size (Bubble Size)
    ...
    # Iterate to find ranges for each brand (Assumes sorted data)
    current_brand = None
    brand_start_row = start_row
    
    # Helper to add series
    def add_brand_series(brand, start, end):
        # Title: Brand Name (Taking cell value from first row of the group)
        # We can just use the string 'brand' as title, or reference the cell. 
        # Using string title for simplicity, or we can use Reference to the first cell.
        title_ref = Reference(ws, min_col=col_brand, min_row=start, max_row=start)
        
        y_values = Reference(ws, min_col=col_y, min_row=start, max_row=end)
        x_values = Reference(ws, min_col=col_x, min_row=start, max_row=end)
        size_values = Reference(ws, min_col=col_size, min_row=start, max_row=end)
        
        series = Series(values=y_values, xvalues=x_values, zvalues=size_values, title=title_ref)
        chart.series.append(series)

    # Scan rows
    # data_range is max_row
    for row in range(start_row, data_range + 1):
        brand_val = ws.cell(row=row, column=col_brand).value
        
        if brand_val != current_brand:
            if current_brand is not None:
                # Close previous group
                add_brand_series(current_brand, brand_start_row, row - 1)
    ...
    # Add last group
    if current_brand is not None:
        add_brand_series(current_brand, brand_start_row, data_range)
        
    chart.width = 25 # Wider for matrix
    chart.height = 15
    return chart


# ... (Existing create_bar_chart function) ...

# ... inside generate_excel loop ...
        num_series = num_cols - 1
        if chart_type == 'line':
            chart = create_line_chart(ws, chart_title, end_row, 2, num_series)
        elif chart_type == 'bubble':
             chart = create_bubble_chart(ws, chart_title, end_row, 2, num_cols)
        else:
