    ...
    for i, chunk in enumerate(chunks):
        ...
        for r, row_data in enumerate(chunk):
            for c, header in enumerate(headers):
                ...
                if any(x in h_str for x in ['cantidad', 'cant.', 'year', 'año', 'mes', 'segmento', 'marca', 'modelo', 'id', 'category']):
                    is_currency = False
                elif any(x in h_str for x in ['precio', 'price', 'monto', 'valor', 'bono', 'lista', 'avg', 'min', 'max']):
                    is_currency = True
                ...
                tf.font.name = "Avenir Light"
                tf.font.size = Pt(9)
                tf.alignment = PP_ALIGN.RIGHT if (is_currency or is_percent) else PP_ALIGN.LEFT
